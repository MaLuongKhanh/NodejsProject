...
(function (window, document, undefined) {
    ...
    function Skrollr(options) {
        ...
        _smoothScrollingDuration =
            options.smoothScrollingDuration ||
            DEFAULT_SMOOTH_SCROLLING_DURATION;
        ...
        _addEvent(window, 'resize orientationchange', function () {
            ...
            if (
                height !== _lastViewportHeight ||
                width !== _lastViewportWidth
            ) {
    ...
    Skrollr.prototype.refresh = function (elements) {
            ...
            for (; attributeIndex < attributesLength; attributeIndex++) {
                ...
                if (attr.name === 'data-anchor-target') {
                    ...
                    if (anchorTarget === null) {
                        throw (
                            'Unable to find anchor target "' + attr.value + '"'
                        );
                    }
        ...
        for (; elementIndex < elementsLength; elementIndex++) {
            var sk =
                _skrollables[
                    elements[elementIndex][SKROLLABLE_ID_DOM_PROPERTY]
                ];
	...
	*/
            ...
            function (e) {
                ...
                switch (e.type) {
                    ...
                    case EVENT_TOUCHEND:
                        ...
                        if (distance2 < 49) {
                            if (
                                !rxTouchIgnoreTags.test(initialElement.tagName)
                            ) {
                        ...
                        if (targetTop > _maxKeyFrame) {
                            targetRatio =
                                (_maxKeyFrame - targetTop) / targetOffset;
        ...
        if (_scrollAnimation) {
            ...
            if (now >= _scrollAnimation.endTime) {
                ...
                progress = _scrollAnimation.easing(
                    (now - _scrollAnimation.startTime) /
                        _scrollAnimation.duration,
                );

                renderTop =
                    (_scrollAnimation.startTop +
                        progress * _scrollAnimation.topDiff) |
                    0;
            }
        ...
        //Smooth scrolling only if there's no animation running and if we're not forcing the rendering.
            ...
            if (now <= _smoothScrolling.endTime) {
                ...
                progress = easings.sqrt(
                    (now - _smoothScrolling.startTime) /
                        _smoothScrollingDuration,
                );

                renderTop =
                    (_smoothScrolling.startTop +
                        progress * _smoothScrolling.topDiff) |
                    0;
            }
        }
    ...
    var _calcInterpolation = function (val1, val2, progress) {
        ...
        for (; valueIndex < val1Length; valueIndex++) {
            ...
            interpolated[valueIndex] =
                val1[valueIndex] +
                (val2[valueIndex] - val1[valueIndex]) * progress;
        }
    ...
    var _detect3DTransforms = function () {
        ...
        var has3D =
            ...
            (computedTransformWithPrefix &&
                computedTransformWithPrefix !== 'none');
    ...
    var _reflow = function () {
        ...
        if (_forceHeight && !_isMobile) {
            //"force" the height.
            body.style.height =
                _maxKeyFrame + documentElement.clientHeight + 'px';
        }

        //The scroll offset may now be larger than needed (on desktop the browser/os prevents scrolling farther than the bottom).
        if (_isMobile) {
            _instance.setScrollTop(
                Math.min(_instance.getScrollTop(), _maxKeyFrame),
            );
        } else {
